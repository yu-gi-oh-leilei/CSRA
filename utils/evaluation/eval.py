...
def evaluation(result, types, ann_path, logger):
    ...
    logger.info("mAP: {:4f}".format(np.mean(aps)))
    logger.info("CP: {:4f}, CR: {:4f}, CF1 :{:4F}".format(CP, CR, CF1))
    logger.info("OP: {:4f}, OR: {:4f}, OF1 {:4F}".format(OP, OR, OF1))
